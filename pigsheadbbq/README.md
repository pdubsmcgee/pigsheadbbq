...
The homepage now links to `/menu.pdf` and `/catering-menu.pdf`, both generated server-side from Google Sheets so visitors never need direct sheet access.
...
The homepage menu buttons open `/menu.pdf` (weekly) and `/catering-menu.pdf`, generated by the Flask server from Google Sheet data.
